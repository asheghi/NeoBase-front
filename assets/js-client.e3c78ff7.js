import{G as o,o as t,c as n,f as s}from"./index.3088f20f.js";const a={class:"markdown-body"},c=s(`<p>this client is an abstraction over our REST APIs, you can learn more about each method in its corresponding API document.</p><h2>Getting Started</h2><p>it\u2019s extremely easy to start working with NeoBase via this client.</p><h4>Create Project</h4><p>if you haven\u2019t registered, create your account at <a href="https://neobase.uk/register">register</a> page. when you\u2019re logged in navigate to <a href="https://neobase.uk/register">dashboard</a>, click on \u201CNew Project\u201D and enter your project name.</p><p>note: project names are unique, so try another one if it was already taken.</p><h2>Install Client</h2><p>to add client on existing project just run the following command to add client to your project dependencies.</p><pre><code class="language-bash">npm i --save @neobase/client
...
const client = getClient(&quot;project-name&quot;, {
  baseUrl: &quot;https://neobase.darkube.app/api&quot;,
});
...
</code></pre><h3>Create</h3><p>create new document in collection.</p><p><strong>Example</strong>: <code class="">create a new todo item</code></p><pre><code class="language-js">const { data: todo } = await Todos.create({
  task: &quot;read the docs&quot;,
  done: false,
});
console.log(todo);
...
</code></pre><p>used to fetch an array of documents of collection. it calls <code class="">find</code> API.</p><p><strong>Example</strong>: <code class="">get all todos</code></p><pre><code class="language-js">const todos = await Todos.find();
</code></pre><p><strong>note</strong>: find() is a chainable method and to execute query you must call \`\` at then end of chain.</p><h4>Filtering</h4><p>just like MongoDB\u2019s <code class="">db.colleciton.find</code> method, to filter documents pass a filter object.</p><p><strong>Example</strong>: <code class="">find todos with status done === true</code></p><pre><code class="language-js">const todos = await Todos.find({ done: true });
</code></pre><h4>Projection</h4><p>select fields you want to be returned.</p><p><strong>Example</strong>: <code class="">get only name of todos</code></p><pre><code class="language-js">const todoNames = await Todos.find().projection(&quot;-_id name&quot;);
</code></pre><p><strong>note</strong>: in mongodb <code class="">_id</code> field is always selected in documents by default, so we need to pass <code class="">-_id</code> in project to not include it.</p><h4>Limit, Skip</h4><p>used for paginating documents. <code class="">limit</code> determines how many documents will be returned. <code class="">skip</code> determines how many documents will be skipped in query.</p><pre><code class="language-js">const page1 = await Todos.find().limit(10);
const page2 = await Todos.find().limit(10).skip(10);
const page3 = await Todos.find().limit(10).skip(20);
</code></pre><h4>Sort</h4><p>used to sort returning document based on one or more fields.</p><p><strong>Example</strong>: <code class="">get todos sorted by created date in descending order</code></p><pre><code class="language-js">const sortedTodos = await Todos.find().sort({ created_at: -1 });
...
</code></pre><p>gets a single document from collection.</p><h4>Filter</h4><p><strong>Example</strong> <code class="">get a todo with id of &#39;asdfasdf&#39;</code></p><pre><code class="language-js">const { data: todo } = Todos.findOne({ _id: &quot;adsfadsf&quot; });
console.log(todo); // { _id: &#39;...&#39;, name: &#39;...&#39;, ...}
</code></pre><h4>Projection</h4><p>select fields you want to be returned.</p><p><strong>Example</strong>: <code class="">get only name and status of todo with code 21</code></p><pre><code class="language-js">const { data: todo } = await Todos.findOne({ code: 21 })
  .projection(&quot;-_id name done&quot;)
  ;
...
</code></pre><h3>Update</h3><p>syntax: <code class="">Collection.update(filter,update) : Promise</code></p><p>updates a document in collection.</p><p><strong>Example</strong>: <code class="">set status of a task to done</code></p><pre><code class="language-js">const { data: result } = await Todos.update(
  { _id: &quot;id-of-todo&quot; },
  { $set: { done: true } }
);
...
</code></pre>`,93),d=[c],u={},g="",h=o({__name:"js-client",setup(i,{expose:e}){return e({frontmatter:{},excerpt:void 0}),(r,l)=>(t(),n("div",a,d))}});export{h as default,g as excerpt,u as frontmatter};
